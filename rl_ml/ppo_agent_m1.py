"""
M1 Mac Optimized PPO Agent for Forex Trading
Uses MPS (Metal Performance Shaders) for GPU acceleration on Apple Silicon
PPO is known for stability and good performance in trading environments
"""
import torch
...
class M1OptimizedPPOCallback(BaseCallback):
    """M1 Mac optimized callback with efficient logging for PPO"""
    ...
    def _on_step(self) -> bool:
        ...
        if self.step_count % self.log_interval == 0:
            if len(self.episode_rewards) > 0:
                ...
                logger.info(f"PPO Step {self.step_count}: "
...
class ForexPPOAgentM1:
    ...
    def __init__(self, env, config=None):
        ...
        if self.device == "mps":
            ...
            logger.info("🚀 M1 Mac PPO optimizations enabled!")
    ...
    def create_model(self) -> PPO:
        """Create M1 optimized PPO model"""
        
        # M1 optimized policy network architecture
        policy_kwargs = {
            'net_arch': [256, 256],  # PPO often benefits from larger networks
            'activation_fn': torch.nn.Tanh,  # Tanh often works better for PPO
        ...
        if self.device == "mps":
            ...
            logger.info("Using CPU with M1 optimizations for stable-baselines3 PPO")
        else:
        ...
        self.model = PPO(
            ...
            n_steps=2048,  # PPO specific: steps per rollout
            batch_size=64,  # PPO specific: minibatch size
            n_epochs=10,  # PPO specific: optimization epochs per rollout
            gamma=self.config.gamma,
            gae_lambda=0.95,  # PPO specific: GAE parameter
            clip_range=0.2,  # PPO specific: clipping parameter
            clip_range_vf=None,  # PPO specific: value function clipping
            normalize_advantage=True,  # PPO specific: advantage normalization
            ent_coef=0.0,  # PPO specific: entropy coefficient
            vf_coef=0.5,  # PPO specific: value function coefficient
            max_grad_norm=0.5,  # PPO specific: gradient clipping
        ...
        logger.info(f"PPO model created for device: {actual_device}")
        return self.model
    
    def train(self, total_timesteps: int = None) -> Dict[str, Any]:
        """Train the M1 optimized PPO agent"""
        ...
        callback = M1OptimizedPPOCallback(log_interval=self.config.log_interval)
        
        logger.info(f"Starting M1 optimized PPO training for {total_timesteps} timesteps...")
        ...
        try:
            ...
            logger.info("M1 optimized PPO training completed successfully!")
            ...
            logger.error(f"M1 PPO training failed: {e}")
            raise
    
    def evaluate(self, n_episodes: int = 5, deterministic: bool = True) -> Dict[str, Any]:
        """Evaluate the M1 trained PPO agent"""
        if self.model is None:
            raise ValueError("Model must be trained before evaluation")
        
        episode_results = []
        logger.info(f"Evaluating M1 PPO agent for {n_episodes} episodes...")
        ...
        for episode in range(n_episodes):
            ...
            logger.info(f"PPO Episode {episode + 1}: "
    ...
    def _evaluate_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"M1 PPO performance evaluation failed: {e}")
            return {}
    
    def save_model(self, filename: str = None):
        """Save the M1 trained PPO model"""
        if self.model is None:
            raise ValueError("No model to save")
        
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"models/ppo_forex_m1_model_{timestamp}.zip"
        
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        
        self.model.save(filename)
        logger.info(f"M1 PPO model saved to {filename}")
    ...
    def load_model(self, filename: str):
        """Load a M1 trained PPO model"""
        try:
            self.model = PPO.load(filename, env=self.env)
            logger.info(f"M1 PPO model loaded from {filename}")
            ...
            try:
                ...
                logger.warning(f"PPO training history not found: {history_filename}")
                
        except Exception as e:
            logger.error(f"Failed to load M1 PPO model: {e}")
            raise
...
if __name__ == "__main__":
    # Test M1 PPO agent creation
    from trading_env import ForexTradingEnv
    from data_fetcher import OandaDataFetcher
    from technical_indicators import TechnicalIndicators
    
    logger.info("Testing M1 PPO agent creation...")
    
    # Check M1 availability
    if torch.backends.mps.is_available():
        logger.info("✅ M1 MPS available and ready for PPO!")
    else:
        logger.info("❌ M1 MPS not available, using CPU for PPO")
    
    print("M1 PPO agent module loaded successfully!")